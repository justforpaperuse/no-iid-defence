...
for ep in range(epoch):
    client_vars_sum = []
    choose_num = math.ceil(Client_num * CLIENT_RATIO_PER_ROUND)
    random_clients = np.random.permutation(Client_num)[:choose_num]
    ...
    global_vars = (np.sum(client_vars_sum, axis=0)/len(random_clients)).tolist()
    Client.set_weights(global_vars)
